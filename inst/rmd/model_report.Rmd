...
<a href = "./index.html"><< Back to BirdFlow model collection</a>
<hr>
```
...
real_track_stat_path <-   file.path(params$output_path, 'real_track_stats.rds')
if(file.exists(real_track_stat_path)){
  mdf <- dplyr::bind_rows(
    eval_metrics[i, c('straightness', 'length', 'displacement', 'n_stopovers')],
    as.data.frame(readRDS(real_track_stat_path))
  )
  ...
  x <- abs((mdf$n_stopovers[2] - mdf$n_stopovers[1])/mdf$n_stopovers[1])
  out_color <- convert_to_color_category(x, breaks = c(0, 0.2, 0.4, Inf), labels = c("green", "yellow", "red"))
  qdf[out_row, 'color'] <- out_color
}
...
# Drop rows for metrics that weren't used
qdf <-  qdf[!is.na(qdf$assessment), , drop = FALSE]
...
cols_3d <- rep('gray', nrow(eval_metrics))
cols_3d[i] <- 'red'

rgl::plot3d(
  x = eval_metrics$de_ratio, y = eval_metrics$obs_prop, z = eval_metrics$dist_pow,
  col = cols_3d,
  type = 's',
...
```{r hypers, echo = FALSE}
knitr::kable(
  eval_metrics[i, c('de_ratio', 'obs_prop', 'dist_pow', 'dist_weight', 'ent_weight', 'obs_weight')]
  )
```
...
knitr::kable(
  signif(eval_metrics[i, c('end_traverse_cor', 'pit_row', 'pit_col', 'pit_in_95')],
         digits = 3)
  )

# Plots

pit_data_path <- file.path(params$output_path, 'pit_data', paste0(sub('\\.hdf5$', '', eval_metrics$model[i]), '_pit.rds'))
...
# Table
knitr::kable(
  round(eval_metrics[i, c('d_pit_row', 'd_pit_col', 'd_pit_in_95', 'pit_d', 'etc_d', 'str_d', 'nso_d', 'overall_des')],
        digits = 3),
...
# Graph
d_vars <- grep('^d_|_d$|overall_des$', names(eval_metrics), value = TRUE)
df <- eval_metrics[,d_vars]
...
```{asis back_rel_link, eval = is_html}
<br>
<a href = "./index.html"><< Back to BirdFlow model collection</a>
