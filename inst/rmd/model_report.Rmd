...
Hyperparameters passed to model fitting process
...
- model evaluation season: `r params$season`
...
```{r desirability, echo = FALSE, message = FALSE}
# Table
knitr::kable(
...
df$color[i] <- 'red'
df <- tidyr::pivot_longer(df, d_vars, names_to = 'variable', values_to = 'desirability')
df$variable <- factor(df$variable, levels = d_vars)

ggplot2::ggplot(df, ggplot2::aes(x=variable, y=desirability), shape = 19) +
  ggplot2::geom_dotplot(ggplot2::aes(fill = color), binaxis='y', stackdir='center', dotsize = 0.3, show.legend = FALSE, stroke = NA) + 
