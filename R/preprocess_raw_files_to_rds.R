...
process_file_set <- function(collapse_list_item){
  ...
  for (species in unique(na.omit(df$EBIRDST_CODE))){
    print(species)
    df %>% filter(EBIRDST_CODE == species) %>% saveRDS(file.path(Sys.getenv('HOME'), 'banding_raw_rds', paste0(species, '.rds')))
  }
}
...
collapse_list <- lapply(collapse_list, function(i) file.path(banding_data_dir, paste0('NABBP_2022_grp_', sprintf('%02d', i), '.csv')))

if (!dir.exists(file.path(Sys.getenv('HOME'), 'banding_raw_rds'))) {dir.create(file.path(Sys.getenv('HOME'), 'banding_raw_rds'))}
