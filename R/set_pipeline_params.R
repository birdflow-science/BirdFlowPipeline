...
#' names.
#' @param grid_search_type Either `"new"` if the search uses the newer 
#'  parameterization of `dist_pow`, `obs_prop`, and `de_ratio`; or `"old"` for
#'  the original parameterization of `dist_pow`, `dist_weight`, and 
#'  `ent_weight`.  In either case there is a hidden parameter
#'   `obs_weight` which is always set to 1.
#' @param grid_search_list These parameters control the loss function used by 
...
#'    be used  - which two depends on  `grid_search_type`. 
#'    The default values are vectors appropriate for `batch_flow()` and are
#'    the set of values that will be combined factorially in the grid 
#'    search.  
#'    
#'    *New* and *Old* indicate which setting of `grid_search_type` they are 
#'    used with.
#'   \describe{
...
#'    function.}
#'  }
#' @param hdf_path The path to the directory in which to store preprocessed
#'    models. **Change to "preprocess_dir? - ebp**
#' @param base_output_path  The base path for fitted models, model reports,
#'  and other output.
#' @param output_fullname Initially `NULL` this is set by 
#' [preprocess_speceies_warpper] to `"<species>_<res>km<suffix>"`. 
#' @param output_path Initially `NULL` this is set by
#' [preprocess_speceies_warpper] to `"<base_output_dir>_<output_fullname>"`.
...
#' }
#' @param clip This is passed to [preprocess_species()] to define a clipping
#' polygon to use while preprocessing - only areas within the polygon are 
#' included in the model.  Although that function takes multiple formats, 
...
#' Note some parameters are updated or set later in the process by 
